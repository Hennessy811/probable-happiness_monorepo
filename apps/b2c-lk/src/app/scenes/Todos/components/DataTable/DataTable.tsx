    ...
    },
    {
        ...
        render: (text: string, record: IToDo) => (
            <div className={styles.ButtonRow} key={record.id}>
                ...
                <Button type="primary" className={styles.ProlongateButton}>Продлить</Button>
            </div>
        ),
    },
];
...
const DataTable = () => {
    const {data} = useSelector((state: IStore) => state.todos);

    return (
        <div className={styles.DataTable}>
            <Title level={3} disabled={!data}>Текущие задачи</Title>
